define(function(require, exports, module){
    ...
    function submit(queryStr){
        ...
        $.ajax({
            url : '/cgi-bin/luci/admin/set_ntp_config',
            type: 'POST',
            data: queryStr,
    ...
    function showWidget($container, sysDateTimes, dst_switch, Timezones, sntp_enables, server1s, server2s, server3s){
        ...
        //不要瞎搞！时间从后台获取到秒数后前端转换。不要在前端对时区做任何操作！
        var sysDate=new Date(eval(sysDateTimes) * 1000);  
    ...
    function display($container){
        ...
        Translate.preLoadDics(dicNames, function(){ 	
            ...
            // 加载路径导航模板模块
            $.ajax({
                url : '/cgi-bin/luci/admin/clockManage',
                type: 'GET',
                success : function(result){
                    result = JSON.parse(result)
                    console.log(result);
