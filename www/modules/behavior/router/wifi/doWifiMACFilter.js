define(function(require, exports, module){
	...
	exports.display = function(){
			...
			var Translate = require('Translate'); 
		 	var dicNames = ['common','doWifiMacFilter']; 
		 	Translate.preLoadDics(dicNames, function(){
				...
				// 加载标签页
				var tabsList = [
					{"id" : "1", "title" : '2.4G '+tl('wifiMACFilter')},
					{"id" : "2", "title" : '5G '+tl('wifiMACFilter')}
				];
				// 生成标签页，并放入页面中
				Tabs.displayTabs(tabsList);
				$('a[href="#1"]').click(function(event) {
					Path.changePath($(this).text());
					require.async('./display24MACFilter', function(obj){		
						obj.display($('#1'));
					});
				});
				$('a[href="#2"]').click(function(event) {
					Path.changePath($(this).text());
					require.async('./display5MACFilter', function(obj){		
						obj.display($('#2'));
					});
				});
