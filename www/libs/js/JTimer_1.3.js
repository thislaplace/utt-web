...
var JTC = (function () {
    ...
    config = {
        dayBgColor: ['#B9CCF9', '#FFFFFF', '#B9CCF9'], //鼠标移动的颜色
        dayColor: ['#0D789C', '#000000', '#959595'], //日期字体颜色:1.上月和下月的日期; 2.本月; 3.不可选时
    ...
    events = {
        ...
        show: function (top, left) {
            ...
            /* MC：根据可视窗口 高度调整日期选择器的显示方式*/
			var _dh = div.style.height;
			_dh = parseInt(_dh.substr(0,_dh.length-2));
			_bdh = parseInt(document.documentElement.clientHeight);
			if(Number(top)+_dh > _bdh){
				top = top - _dh -30;
			}
			/* MC end*/
