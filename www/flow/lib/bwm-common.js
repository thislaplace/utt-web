...
var colorX = 0;
var colors = [
	['ÂÌ &amp; À¶', '#118811', '#6495ed'], ['À¶ &amp; ³È', '#003EBA', '#FF9000'],
	['À¶ &amp; ºì', '#003EDD', '#CC4040'], ['À¶', '#22f', '#225'], ['»Ò', '#000', '#999'],
	['ºì &amp; ºÚ', '#d00', '#000']
];
function xpsb(byt){
/* REMOVE-BEGIN
...
function showTab(name){
	...
	cookie.set(cprefix + _cookieName, ifname, 14);
	tabHigh(name);
...
function loadData(){
	...
	if (!speed_history) {
		...
		tabs = tabs.sort(function(a, b) {
			...
			a[1]=a[1].toString();
			b[1]=b[1].toString();
			wF1=a[1].toUpperCase().indexOf('W');/*ÊÇ·ñwan¿Ú*/
			wF2=b[1].toUpperCase().indexOf('W');
			/*ÏÈÅÅwan¿Ú¡£ÇÒÅÅÇ°Ãæ*/
			if(wF1>=0)
			{
				if(wF2>=0)
			...
			/* ÔÙÅÅ3G */
			else if (a[1].toUpperCase() == "3G") {
				return -1;
			} else if (b[1].toUpperCase() == "3G") {
				return 1;
			}
			/* ÔÙÅÅ APClient */
			else if (a[1].toUpperCase() == "APCLIENT") {
				return -1;
			} else if (b[1].toUpperCase() == "APCLIENT") {
				return 1;
			}
			/*ÔÙÏÔÊ¾lan×Ü¿Ú*/
			else if(a[1].toUpperCase().indexOf('L')>=0)
			{
				return -1;
			}
			else if(b[1].toUpperCase().indexOf('L')>=0)
			{
				return 1;
			}
			/*×îºóÅÅÐò¸÷¸ö¶Ë¿Ú*/
	...
	if (((name = cookie.get(cprefix + _cookieName)) != null) && ((speed_history[name] != undefined))) {
