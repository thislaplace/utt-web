...
var form = {
	...
	submit: function(fom, async, url) {
		...
		this.xhttp.get(url ? url : fom.action);
	},
...
var cookie = {
	set: function(key, value, days) {
	  document.cookie = 'utt_' + key + '=' + value + '; expires=' + (new Date(new Date().getTime() + ((days ? days: 14) * 86400000))).toUTCString() + '; path=/';
	},   
	get: function(key) {
	  var r = ('; ' + document.cookie + ';').match('; utt_' + key + '=(.*?);');
	  return r ? r[1] : null;
	},   
	unset: function(key) {
	  document.cookie = 'utt_' + key + '=; expires=' + (new Date(1)).toUTCString() + '; path=/';
	}    
};
